...
handler.enter = function(msg, session, next) {
	var self = this;
	var rid = msg.rid;//房间
	var userX = Math.ceil(Math.random()*msg.stageWidth);//用户位置
	...
	session.bind(uid);//将session与该用户id建立一个映射关系， key -- value     （取值方法：var username = session.uid.split('*')[0];）
	session.set('rid', rid);//给session添加一个属性， 提供用户更改session的机会。（取值方法：var rid = session.get('rid');）
	...
	self.app.rpc.chat.chatRemote.add(session, uid, self.app.get('serverId'), rid,userX , true, function(users){
		next(null, {
			users:users,
			userX:userX //这是返回给自己的，在这个self.app.rpc.chat.chatRemote.add方法里，还有个返回给侦听了'onAdd'的用户的
		});
	});
};
